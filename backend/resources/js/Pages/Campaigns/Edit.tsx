...
export default function CampaignEdit({ campaign, groups, available_statuses }: CampaignEditProps) {
    const { data, setData, put, processing, errors } = useForm({
        title: campaign.title,
        synopsis: campaign.synopsis ?? '',
        status: campaign.status,
        region_id: campaign.region_id ? campaign.region_id.toString() : '',
        default_timezone: campaign.default_timezone,
        start_date: campaign.start_date ?? '',
        end_date: campaign.end_date ?? '',
        turn_hours: campaign.turn_hours ?? 24,
    });

    const regions = useMemo(() => groups.flatMap((group) => group.regions), [groups]);
    ...
    return (
        <AppLayout>
            ...
            <div className="mb-6">
                <h1 className="text-3xl font-semibold text-zinc-100">Update campaign</h1>
                <p className="mt-2 text-sm text-zinc-400">
                    Adjust the campaign synopsis, cadence, or status as your story evolves.
                </p>
            </div>
            ...
            <form onSubmit={submit} className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <Link href={route('campaigns.show', campaign.id)} className="text-sm text-zinc-400 hover:text-zinc-200">
                        Back to campaign
                    </Link>
                </div>
            </form>
